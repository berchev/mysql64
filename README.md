...
## Description:
Downloading the content of this repo, you will have all needed configuration files required to build a **mysql64 box** and to test it with tool **Kitchen**
...
### Instructions HOW to build the mysql64 box
- Make sure you have at least 2GB free space on your drive before start building of the box.
- Download the content of **berchev/mysql64** repository: `git clone https://github.com/berchev/mysql64.git`
- Change to downloaded **mysql** directory: `cd mysql64`
...
### Instructions HOW to use the already created mysql64 box
...
- On your terminal type: `vagrant box add --name <box_name> ubuntu-1604-vbox.box` , where `<box_name>` is the name which you pick for your box. For example: mysql64 (it is good approach to use lowercase letters in Linux)
- On your terminal type: `vagrant init mysql64`, where mysql64 is the name which we picked in step 1. 
It can be any other name!
`vagrant init mysql64` command will place to the current directory `Vagrantfile`.
...
Note that if your output box file is `ubuntu-1604-vbox.box` and your box name added to vagrant is `mysql64`, you do not need to change anything
